...
//-----------------------------------------------------------------------------
class C_Holdout : public CHudNumericDisplay, public CHudElement
{
	DECLARE_CLASS_SIMPLE( C_Holdout, CHudNumericDisplay );
  DECLARE_CLIENTCLASS(); // this is a client representation of a server class

public:
	C_Holdout( const char *pElementName );
	void Init( void );
	void VidInit( void );
	void Reset();
