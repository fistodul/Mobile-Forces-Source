...
ConVar	sk_max_gauss		( "sk_max_gauss", "150", FCVAR_REPLICATED );
...
{ 
	...
	if ( IsTeamplay() )
	return "Mobile Forces Source - Squad Deathmatch"; //SecobMod__ChangeME!
	else if ( !IsTeamplay() )
	return "Mobile Forces Source - Deathmatch";
	else
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("Portable_healthkit",			DMG_BURN,					TRACER_NONE,			"0",		"0",		"sk_max_healthkit",		0, 0);
#ifdef HL2_EPISODIC
...
void CHL2MPRules::InitTrollAIRelationships( void )
{
	int i, j;
