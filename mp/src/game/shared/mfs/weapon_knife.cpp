...
#define	KNIFE_BEAM_MATERIAL		"sprites/lgtning.vmt"
#define KNIFE_GLOW_MATERIAL		"sprites/light_glow02_add"
#define KNIFE_GLOW_MATERIAL2	"effects/blueflare1"
#define KNIFE_GLOW_MATERIAL_NOZ	"sprites/light_glow02_add_noz"
...
void CWeaponKnife::Precache()
{
	...
	PrecacheModel( KNIFE_BEAM_MATERIAL );
...
//-----------------------------------------------------------------------------
void CWeaponKnife::SetStunState( bool state )
{
	m_bActive = state;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponKnife::GetStunState( void )
{
	return m_bActive;
}
...
//-----------------------------------------------------------------------------
void C_WeaponKnife::SetupAttachmentPoints( void )
{
...
//-----------------------------------------------------------------------------
int C_WeaponKnife::DrawModel( int flags )
{
	if ( ShouldDraw() == false )
		return 0;
...
//-----------------------------------------------------------------------------
void C_WeaponKnife::ClientThink( void )
{
	if ( InSwing() == false )
	{
	...
	if ( ShouldDrawUsingViewModel() )
	{
		...
		{		
			...
			beamInfo.m_pszModelName = KNIFE_BEAM_MATERIAL;
...
//-----------------------------------------------------------------------------
void C_WeaponKnife::OnDataChanged( DataUpdateType_t updateType )
{
...
//-----------------------------------------------------------------------------
RenderGroup_t C_WeaponKnife::GetRenderGroup( void )
{
...
// Purpose: Tells us we're always a translucent entity
//-----------------------------------------------------------------------------
bool C_WeaponKnife::InSwing( void )
{
...
//-----------------------------------------------------------------------------
void C_WeaponKnife::DrawThirdPersonEffects( void )
{
	Vector	vecOrigin;
	QAngle	vecAngles;
	float	color[3];
	...
	IMaterial *pMaterial = materials->FindMaterial( KNIFE_GLOW_MATERIAL, NULL, false );
	...
	if ( InSwing() )
	{
		pMaterial = materials->FindMaterial( KNIFE_GLOW_MATERIAL2, NULL, false );
		pRenderContext->Bind( pMaterial );
...
// Purpose: Draw our special effects
//-----------------------------------------------------------------------------
void C_WeaponKnife::DrawFirstPersonEffects( void )
{
	Vector	vecOrigin;
	QAngle	vecAngles;
	float	color[3];
	...
	CMatRenderContextPtr pRenderContext( materials );
	IMaterial *pMaterial = materials->FindMaterial( KNIFE_GLOW_MATERIAL_NOZ, NULL, false );
...
// Purpose: Draw our special effects
//-----------------------------------------------------------------------------
void C_WeaponKnife::DrawEffects( void )
{
	if ( ShouldDrawUsingViewModel() )
	{
...
//-----------------------------------------------------------------------------
void C_WeaponKnife::ViewModelDrawn( C_BaseViewModel *pBaseViewModel )
{
...
//-----------------------------------------------------------------------------
void KnifeImpactCallback( const CEffectData &data )
{
...
DECLARE_CLIENT_EFFECT( "StunstickImpact", KnifeImpactCallback );
