...
ConVar	sk_plr_dmg_sniper("sk_plr_dmg_sniper", "100", FCVAR_REPLICATED);
ConVar	sk_npc_dmg_sniper("sk_npc_dmg_sniper", "10", FCVAR_REPLICATED);
ConVar	sk_max_sniper("sk_max_sniper", "12", FCVAR_REPLICATED);

#ifndef CLIENT_DLL

extern ConVar sk_plr_dmg_sniper;
extern ConVar sk_npc_dmg_sniper;
...
// Crossbow Bolt
//-----------------------------------------------------------------------------
class CSniper : public CBaseCombatCharacter
{
	DECLARE_CLASS( CSniper, CBaseCombatCharacter );
...
public:
	...
	static CSniper *BoltCreate( const Vector &vecOrigin, const QAngle &angAngles, int iDamage, CBasePlayer *pentOwner = NULL );
...
LINK_ENTITY_TO_CLASS( sniper_bolt, CSniper );

BEGIN_DATADESC( CSniper )
	// Function Pointers
...
IMPLEMENT_SERVERCLASS_ST( CSniper, DT_Sniper )
END_SEND_TABLE()

CSniper *CSniper::BoltCreate( const Vector &vecOrigin, const QAngle &angAngles, int iDamage, CBasePlayer *pentOwner )
{
	// Create a new entity with CSniper private data
	CSniper *pBolt = (CSniper *)CreateEntityByName( "crossbow_bolt" );
...
//-----------------------------------------------------------------------------
bool CSniper::CreateVPhysics( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
unsigned int CSniper::PhysicsSolidMaskForEntity() const
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSniper::CreateSprites( void )
{
...
void CSniper::Spawn( void )
{
	...
	SetTouch( &CSniper::BoltTouch );

	SetThink( &CSniper::BubbleThink );
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CSniper::BoltTouch( CBaseEntity *pOther )
{
...
{
#ifndef CLIENT_DLL
	UTIL_PrecacheOther( "sniper_bolt" );
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSniper::FireBolt( void )
{
	if ( m_iClip1 <= 0 )
	{
		if ( !m_bFireOnEmpty )
	...
	CSniper *pBolt = CSniper::BoltCreate( vecSrc, angAiming, GetHL2MPWpnData().m_iPlayerDamage, pOwner );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSniper::CreateChargerEffects( void )
{
#ifndef CLIENT_DLL
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Input  : skinNum - 
//-----------------------------------------------------------------------------
void CWeaponSniper::SetSkin( int skinNum )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSniper::DoLoadEffect( void )
{
	SetSkin( BOLT_SKIN_GLOW );
...
// Input  : state - 
//-----------------------------------------------------------------------------
void CWeaponSniper::SetChargerState( ChargerState_t state )
{
